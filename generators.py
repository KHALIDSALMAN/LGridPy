...
class GasGenerator(Generator):
    ...
    def __init__(self, name, carrier, p_nom, p_min_pu=0., p_max_pu=1., min_uptime=0., min_downtime=0., ramp_up_limit=1., ramp_down_limit=1., start_up_cost=0., shut_down_cost=0., efficiency_curve=None, efficiency_type=None, fuel_price=1., co2_per_mw=0.517, SFC=0.215):
    ...
    def set_efficiency_curve_parameters(self, preprocessment=False):
        ...
        # Each fitting type will result in different parameters
        fitting_type = self.efficiency_type
        ef_params = []
        
        # Constant fitting for constant efficiency
        ef_params_constant = np.asarray([0, 1/np.mean(y), 0])

        # Quadratic fitting for efficiency curve
        # SciPy least square fitting - setting the fitting to have a > 0 and b > 0 (only positive costs)
        def fitting_function(x, a, b, c):
            return x/(a*x**2 + b*x + c)
        
        def residuals(params, x, y):
            a, b, c = params
            penalization = (int(a < 0) + int(b < 0)) * 1e4
            return y - fitting_function(x, a, b, c) - penalization
        
        ef_params_quadratic, _ = leastsq(func=residuals, x0=(1., 1., 1.), args=(x, y), maxfev=5000)
        
        # Gambiarra fitting
        # ef_params_quadratic = np.polyfit(x, x/y, 2)

        # Choose the parameters according to fitting type of the generator
        if fitting_type == 'constant':
            ef_params = ef_params_constant

        elif fitting_type == 'quadratic':
            ef_params = ef_params_quadratic

        # If performing preprocessment, do not update any parameter, just return the values
        if preprocessment:
            ef_params = ef_params_constant[1]
            return ef_params
...
class WindGenerator(Generator):
    ...
    def set_p_max_pu(self, load):       
        ...
        power_curve = pd.read_excel('15MW_power_curve.xlsx')
